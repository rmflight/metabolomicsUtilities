...
Hunter thinks the correction should be:

$$\frac{(P_{max}/2) - P_{missing}}{P_{max} - P_{missing}}$$

where $P_{missing} = P_{max} - P_{sample}$

```{r correct_data}
max_peaks = max(use_peaks$n_peak)

use_peaks2 = dplyr::select(use_peaks, imf, Sample_Peak, fraction, rank, n_peak, fractional_rank) %>%
  dplyr::mutate(p_missing = max_peaks - n_peak)
use_peaks2 = dplyr::mutate(use_peaks2, correction_factor = ((max_peaks/2) - p_missing) / (max_peaks - p_missing))
use_peaks2 = dplyr::mutate(use_peaks2, corrected_rank = fractional_rank * (1 - correction_factor))
...
OK, this doesn't work, and now I don't know why.
...
```{r correct_data2}
max_peaks = 3500
use_peaks3 = dplyr::select(use_peaks, imf, Sample_Peak, fraction, rank, n_peak, fractional_rank) %>%
  dplyr::mutate(model1 = 1 - (n_peak / max_peaks),
                model2 = n_peak / max_peaks)
use_peaks3 = dplyr::mutate(use_peaks3, corrected_rank = model1 + (model2 * fractional_rank))

ggplot(use_peaks3, aes(x = n_peak, y = corrected_rank)) + geom_point(alpha = 0.5) +
  geom_smooth(method = "lm")
cor.test(use_peaks3$n_peak, use_peaks3$corrected_rank)
```

This still doesn't **remove** the correlation. It does make it smaller (0.15 vs 0.23).
...
removed_keep = dplyr::mutate(removed_keep, max_peaks = max(n_peak),
                             model1 = 1 - (n_peak / max_peaks),
                             model2 = n_peak / max_peaks)
removed_keep = dplyr::mutate(removed_keep, corrected_rank = model1 + (model2 * new_rank))

ggplot(removed_keep, aes(x = n_peak, y = corrected_rank)) + geom_point() + geom_smooth(method = "lm")
cor.test(removed_keep$n_peak, removed_keep$corrected_rank)
```
...
ggplot(removed_keep, aes(x = ratio_peaks, y = ratio_rank)) + geom_point() + geom_smooth(method = "lm")

ratio_fit = lm(ratio_rank ~ ratio_peaks, data = removed_keep)
removed_keep = dplyr::mutate(removed_keep, corrected_rank2 = new_rank / (0.3 + 0.74*ratio_peaks))

ggplot(removed_keep, aes(x = n_peak, y = corrected_rank2)) + geom_point() + geom_smooth(method = "lm")
cor.test(removed_keep$n_peak, removed_keep$corrected_rank2)
```

Neat, this works.

So this makes what we are doing equivalent to:

$$\frac{rank_{observed}}{rank_{true}}=int+slope\times\frac{P_{sample}}{P_{max}}$$

### Apply to Lung Data

Can we apply that same correction to our lung data?

```{r apply_simulated_to_lung}
use_peaks4 = dplyr::select(use_peaks, imf, Sample_Peak, fraction, rank, n_peak, fractional_rank) %>%
  dplyr::mutate(max_peaks = max(n_peak),
                ratio_peaks = n_peak / max_peaks,
                corrected_rank = fractional_rank / (0.3 + 0.74 * ratio_peaks))
ggplot(use_peaks4, aes(x = n_peak, y = corrected_rank)) + geom_point() + 
  geom_smooth(method = "lm")
cor.test(use_peaks4$n_peak, use_peaks4$corrected_rank)
```

That's not zero, but that's **really really close**.
